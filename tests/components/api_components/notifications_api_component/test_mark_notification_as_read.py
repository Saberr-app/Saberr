...
CASES = [
    Case(id="marks found notification read", found=True),
...
async def test_mark_notification_as_read(case: Case, make_notification, mocker):
    found = make_notification(code=NotificationCode.LOGIN, level=LEVEL,
                              status=NotificationStatus.UNREAD) if case.found else None
    ...
    if case.expected_exception is not None:
        with pytest.raises(case.expected_exception):
            await NotificationAPIComponent().mark_notification_as_read(notification_id=999999)
        update.assert_not_awaited()
        return

    await NotificationAPIComponent().mark_notification_as_read(notification_id=found.id)
    update.assert_awaited_once_with(notification_id=found.id, status=NotificationStatus.READ)
